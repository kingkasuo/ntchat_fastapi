...
@app.post("/user/get_login_info", summary="获取登录信息", tags=["User"],
          response_model=models.ResponseModel)
@catch_exception()
async def get_login_info(model: models.ClientReqModel):
    data = client_mgr.get_client(model.guid).get_login_info()
...
@app.post("/contact/search_contacts", summary="查询联系人列表", tags=["Contact"],
          response_model=models.ResponseModel)
@catch_exception()
...
@app.post("/room/get_room_detail", summary="获取指定群详细信息", tags=["Room"],
          response_model=models.ResponseModel)
@catch_exception()
async def get_room_detail(model: models.RoomReqModel):
    data = client_mgr.get_client(model.guid).get_room_detail(model.room_wxid)
    return response_json(1, data)
...
@app.post("/msg/send_xml", summary="发送XML消息(小程序)", tags=["Msg"], response_model=models.ResponseModel)
@catch_exception()
async def msg_send_xml(model: models.SendXmlReqModel):
